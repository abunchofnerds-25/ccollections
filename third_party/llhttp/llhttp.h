 ...
 *
 * Normally `llhttp` would error when the HTTP version in the request or status
 * line is not `0.9`, `1.0`, `1.1` or `2.0`. With this flag the invalid value
 * will be parsed normally.
 *
 * **Enabling this flag can pose a security issue since you will allow
 * unsupported HTTP versions. USE WITH CAUTION!**
 */
LLHTTP_EXPORT
void llhttp_set_lenient_version(llhttp_t* parser, int enabled);

/* Enables/disables lenient handling of additional data received after a message
 * ends and keep-alive is disabled.
 *
 * Normally `llhttp` would error when additional unexpected data is received if
 * the message contains the `Connection` header with `close` value. With this
 * flag the extra data will discarded without throwing an error.
 *
 * **Enabling this flag can pose a security issue since you will be exposed to
 * poisoning attacks. USE WITH CAUTION!**
 */
 ...
 *
 * Normally `llhttp` would error when a CR is not followed by LF when
 * terminating the request line, the status line, the headers or a chunk header.
 ...
 *
 * Normally `llhttp` would error when a LF is not preceded by CR when
 * terminating the request line, the status line, the headers, a chunk header or
 * a chunk data. With this flag only a LF is required to terminate such
 * sections.
 *
 * **Enabling this flag can pose a security issue since you will be exposed to
 * request smuggling attacks. USE WITH CAUTION!**
 ...
 *
 * Normally `llhttp` would error when after a chunk data a CRLF is missing
 * before starting a new chunk. With this flag the new chunk can start
 ...
 */
LLHTTP_EXPORT
void llhttp_set_lenient_optional_crlf_after_chunk(llhttp_t* parser,
                                                  int enabled);

/* Enables/disables lenient handling of spaces after chunk size.
 *
 * Normally `llhttp` would error when after a chunk size is followed by one or
 * more spaces are present instead of a CRLF or `;`. With this flag this check
 * is disabled.
 *
 * **Enabling this flag can pose a security issue since you will be exposed to
 * request smuggling attacks. USE WITH CAUTION!**
 ...
 * rejected, while other control characters are merely 'invalid' and
 * discouraged, and are explicitly allowed by other standards (e.g. WHATWG
 * Fetch) and in surprisingly common use on the web.
 *
