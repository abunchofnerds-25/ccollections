...
static inline void fio_alpn_destroy(alpn_s *obj) {
  if (obj->on_cleanup) obj->on_cleanup(obj->udata_tls);
...
FIO_FUNC inline alpn_s *alpn_default(fio_tls_s *tls) {
  if (!tls || !alpn_list_count(&tls->alpn) || !tls->alpn.ordered) return NULL;
...
FIO_FUNC inline void alpn_select(alpn_s *alpn, intptr_t uuid,
                                 void *udata_connection) {
  if (!alpn || !alpn->on_selected) return;
...
static int fio_tls_alpn_selector_cb(SSL *ssl, const unsigned char **out,
  ...
  if (alpn_list_count(&tls->alpn) == 0) return SSL_TLSEXT_ERR_NOACK;
  ...
  if (ri != 1) {
    ...
    switch (ri) {
      ...
      case SSL_ERROR_WANT_WRITE:
        // FIO_LOG_DEBUG("SSL_accept/SSL_connect %p state:
...
void FIO_TLS_WEAK fio_tls_destroy(fio_tls_s *tls) {
  if (!tls) return;
  REQUIRE_LIBRARY();
  if (fio_atomic_sub(&tls->ref, 1)) return;
  fio_tls_destroy_context(tls);
