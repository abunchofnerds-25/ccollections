...
FIO_INLINE fiobj_type_enum fiobj_type(FIOBJ o) {
  if (!o) return FIOBJ_T_NULL;
  if (o & FIOBJECT_NUMBER_FLAG) return FIOBJ_T_NUMBER;
...
FIO_INLINE const char *fiobj_type_name(const FIOBJ o) {
  if (o & FIOBJECT_NUMBER_FLAG) return "Number";
  if (FIOBJ_IS_ALLOCATED(o)) return FIOBJECT2VTBL(o)->class_name;
  if (!o) return "NULL";
  return "Primitive";
}
...
FIO_INLINE FIOBJ fiobj_dup(FIOBJ o) {
  if (FIOBJ_IS_ALLOCATED(o)) OBJREF_ADD(o);
  return o;
}
...
FIO_INLINE void fiobj_free(FIOBJ o) {
  if (!FIOBJ_IS_ALLOCATED(o)) return;
  if (fiobj_ref_dec(o)) return;
...
FIO_INLINE int fiobj_is_true(const FIOBJ o) {
  if (o & FIOBJECT_NUMBER_FLAG) return ((uintptr_t)o >> 1) != 0;
  if ((o & FIOBJECT_PRIMITIVE_FLAG) == FIOBJECT_PRIMITIVE_FLAG)
...
FIO_INLINE intptr_t fiobj_obj2num(const FIOBJ o) {
  ...
  if (!o || !FIOBJ_IS_ALLOCATED(o)) return o == FIOBJ_T_TRUE;
...
FIO_INLINE fio_str_info_s fiobj_obj2cstr(const FIOBJ o) {
  ...
  if (o & FIOBJECT_NUMBER_FLAG) return fio_ltocstr(((intptr_t)o) >> 1);
  if ((o & FIOBJECT_PRIMITIVE_FLAG) == FIOBJECT_PRIMITIVE_FLAG) {
 ...
 */
FIO_INLINE uint64_t fiobj_obj2hash(const FIOBJ o) {
  if (FIOBJ_TYPE_IS(o, FIOBJ_T_STRING)) return fiobj_str_hash(o);
  if (!FIOBJ_IS_ALLOCATED(o)) return (uint64_t)o;
...
FIO_INLINE double fiobj_obj2float(const FIOBJ o) {
  if (o & FIOBJECT_NUMBER_FLAG) return (double)(fiobj_obj2num(o));
